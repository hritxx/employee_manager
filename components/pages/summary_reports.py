    ...
    with summary_tabs[4]:  
        ...
        with col1:
            view_option = st.selectbox(
                "Select View:",
                ["Basic Summary", "Detailed Analysis", "Department Hierarchy"],
                key="dept_view_option"
            )
        ...
        if view_option == "Basic Summary":
            ...
            dept_data = get_detailed_department_analysis()
        else:
        ...
        if not dept_data.empty:
            ...
            if view_option in ["Basic Summary", "Detailed Analysis"]:
                ...
                if 'total_employees' in dept_data.columns:
                    ...
                    with metric_cols[1]:
                        total_employees = dept_data['total_employees'].sum()
                        st.metric("Total Employees", total_employees)
                    with metric_cols[2]:
                        if 'active_employees' in dept_data.columns:
                            active_employees = dept_data['active_employees'].sum()
                            st.metric("Active Employees", active_employees)
                    with metric_cols[3]:
                        if 'total_projects' in dept_data.columns:
                ...
                if view_option == "Detailed Analysis" and 'active_employee_percentage' in dept_data.columns:
                    ...
                    with viz_col3:
                        ...
                        fig_status = px.bar(
                            dept_data, 
                            x='department_name', 
                            y=['active_employees', 'inactive_employees'],
                            title="Active vs Inactive Employees by Department",
                            barmode='stack'
                        )
                ...
                if 'business_unit' in dept_data.columns:
                    ...
                    bu_summary.columns = [
                        'Total Employees', 
                        'Active Employees' if 'active_employees' in dept_data.columns else 'Total Employees',
                        'Avg Experience (Years)', 
                        'Number of Departments'
                    ]
                    
                    col_bu1, col_bu2 = st.columns([1, 1])
                    with col_bu1:
                    ...
                    with col_bu2:
                        ...
                        fig_bu = px.bar(
                            ...
                            x='business_unit', 
                            y='Total Employees',
                            title="Employee Distribution by Business Unit",
                ...
                if 'level' in dept_data.columns and not dept_data.empty:
                    ...
                    level_dist.columns = ['Level', 'Department Count', 'Total Employees']
                    ...
                    with col_h1:
                        fig_levels = px.bar(
                            ...
                            x='Level', 
                            y='Department Count',
                            title="Departments by Hierarchy Level"
                        )
                        st.plotly_chart(fig_levels, use_container_width=True)
                    
                    with col_h2:
                        fig_emp_levels = px.bar(
                            level_dist, 
                            x='Level', 
                            y='Total Employees',
                            title="Employees by Hierarchy Level"
                        )
                        st.plotly_chart(fig_emp_levels, use_container_width=True)
        ...
        if not dept_data.empty:
            ...
            with export_col1:
                if st.button("📊 Export to CSV", key="export_dept_csv"):
                    csv = dept_data.to_csv(index=False)
                    st.download_button(
                        label="Download CSV",
                        data=csv,
                        file_name=f"department_{view_option.lower().replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                        mime="text/csv"
                    )
